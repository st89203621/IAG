...
for uparea_id in 210213 220214 230215; do
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE capture_day = '$yesterday' AND capture_hour BETWEEN '00' AND '23' AND user_name LIKE '213%' AND uparea_id = '$uparea_id'
            UNION ALL
    ...
    ) tmp WHERE rn = 1;" >> ./output_new/$yesterday/nf_acc_${yesterday}_${uparea_id}.csv &
done
...
for uparea_id in 210213 220214 230215; do
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE capture_day = '$yesterday' AND capture_hour BETWEEN '00' AND '23' AND user_name not LIKE '213%' AND uparea_id = '$uparea_id'
            UNION ALL
        ...
        WHERE nf.capture_time >= radius.capture_time
    ) tmp WHERE rn = 1;" >> ./output_new/$yesterday/nf_fix_acc_${yesterday}_${uparea_id}.csv &
done

wait
echo "NF昨天关联准确度统计完成: $yesterday"
